...
const RemoveObject = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      formData.append('image', input)   // ✅ must match backend "upload.single('image')"
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96'>
        ...
        {!content ? (
          ...
          <div className='mt-3 flex flex-col gap-3'>
            <img src={content} alt="Processed" className='w-full rounded-md'/>
            <button 
              onClick={handleDownload}
              className='bg-[#4A7AFF] text-white px-4 py-2 rounded-lg text-sm hover:bg-[#3a6ce0]'
            >
              Download Image
            </button>
          </div>
